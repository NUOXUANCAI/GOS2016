function [CIvecF,CInu,CIb,CIu,CIbnuT,CIunuT,Tij,SIGMAF,SIGMAnu]=CIlambda_nu_F_nfix(Ti,I,res,V,X,F,Z,MC,vecbeta3,nu,nuT,lambda,Fhat,n,T,K,p,q,d1,d2,d,weights)
...
for i=1:n    
    ...
    elseif weights==1;
        w(:,:,i)=tau(i,1)*mean(MC(:,i));                  %scalar, weight
    end;
    Qx(:,:,i)=(X(:,:,i)*X(:,:,i)')/Tij(i,i);
    Qb3=Qb3+beta3(:,:,i)'*w(:,:,i)*beta3(:,:,i);%KpxKp matrix
end;
Qb3=Qb3/n;
%Defintion of SIGMAnu (for n fixed and T\rightarrow\infty, no thresholding)
Ssum=zeros(K*p,K*p);        %element of the sum
for i=1:n;
...
for t=2:T;                              %See Prop.4
    ...
    stnuT(:,t-1)=sqrt(diag(SIGMAnuT)/(T-1));
